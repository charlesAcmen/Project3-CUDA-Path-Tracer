/**
 * @file rng_compare.cu
 * @brief Standalone RNG comparison: LCG vs Owen-scrambled Halton.
 *
 ...
 *
 * Output (CSV):
 *   pixel,iter,bounce,dim,lcg,halton_owen
 *
 * Two columns for the same (pixel, iter, bounce, dim) tuple:
 *   lcg         — thrust LCG (baseline)
 *   halton_owen — Owen-scrambled Halton (current implementation)
 */
...
static void writeHeader(FILE* f) {
    fprintf(f, "pixel,iter,bounce,dim,lcg,halton_owen\n");
}

static void writeRow(FILE* f,
                     int pixel, int iter, int bounce, int dim,
                     float lcgVal, float owenVal)
{
    fprintf(f, "%d,%d,%d,%d,%.9g,%.9g\n",
            pixel, iter, bounce, dim, lcgVal, owenVal);
}
        ...
        for (int iter = 0; iter < args.numSamples; ++iter) {
            for (int bounce = 0; bounce < args.numBounces; ++bounce) {
                ...
                for (int dim = 0; dim < args.numDims; ++dim) {
                    ...
                    writeRow(f, pixel, iter, bounce, dim, lcgVal, owenVal);
                }
